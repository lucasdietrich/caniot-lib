...
unsafe extern "C" fn trampoline_controler_cb(
    ...
    let verdict = api.on_event(event);
...
impl<D: Driver> Controller<D> {
    ...
    fn as_ptr(&self) -> *const ll::caniot_controller {
        self.controller.as_ref() as *const ll::caniot_controller
    }

    pub fn next_timeout(&self) -> Option<Duration> {
        // SAFETY: We are calling a method that does not mutate the controller.
...
impl<D: Driver> Drop for Controller<D> {
    fn drop(&mut self) {
        // SAFETY: We are dropping the controller, so we can safely call the C function.
