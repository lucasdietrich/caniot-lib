...
impl<D: Driver> Controller<D> {
	...
	fn as_mut_ptr(&mut self) -> *mut ll::caniot_controller { self.controller.as_mut() }
	fn as_ptr(&self) -> *const ll::caniot_controller { self.controller.as_ref() }
	...
	pub fn query(
		...
		let ret = unsafe { ll::caniot_controller_query(self.as_mut_ptr(), did.to_u8(), &mut frame, timeout_ms) };
		FailCode::to_result(ret).map(|handle| Handle::new(handle as u8))
	}
}

impl<D: Driver> Drop for Controller<D> {
	fn drop(&mut self) { unsafe { ll::caniot_controller_deinit(self.as_mut_ptr()) }; }
}

#[cfg(feature = "std")]
impl<D: Driver + AsRawFd> AsRawFd for Controller<D> { fn as_raw_fd(&self) -> std::os::fd::RawFd { self.driver.as_raw_fd() } }

#[cfg(feature = "std")]
impl<D: Driver + AsFd> AsFd for Controller<D> { fn as_fd(&self) -> std::os::fd::BorrowedFd<'_> { self.driver.as_fd() } }
...
mod tests {
	...
	use super::*;
	#[test]
	fn test_controller_init() { let ctrl = Controller::init(DummyDriver, DummyControllerApi); assert!(ctrl.is_ok()); }
}
