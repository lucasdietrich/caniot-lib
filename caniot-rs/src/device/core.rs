...
use crate::{
	device::{api::DeviceApiWrapper, config::StaticConfig, identity::Identity, implementation::DeviceApi},
	did::DeviceId,
	driver::Driver,
	error::FailCode,
	types::Endpoint,
...
impl<D: Driver, A: DeviceApi + 'static> Device<D, A> {
	pub fn init(mut driver: D, did: DeviceId, api: A, config: StaticConfig) -> Result<Device<D, A>, FailCode> {
		...
		Ok(Device { device, config, identity, api, driver })
	}

	#[allow(dead_code)]
	fn as_ptr(&self) -> *const ll::caniot_device { &self.device }

	pub fn run_once(&mut self) -> Result<(), FailCode> { let ret = unsafe { ll::caniot_device_process(&mut self.device) }; FailCode::to_result(ret)?; Ok(()) }

	pub fn next_timeout(&mut self) -> Option<Duration> {
		let ret = unsafe { ll::caniot_device_time_until_process(&mut self.device) };
		match ret { u32::MAX => None, timeout => Some(Duration::from_millis(timeout as u64)) }
	}
	...
	pub fn run(&mut self) -> Result<(), FailCode> { loop { self.run_once()?; } }
	pub fn get_api_mut(&mut self) -> &mut A { self.api.as_mut() }
}

#[cfg(feature = "std")]
impl<D: Driver + AsRawFd, A: DeviceApi + 'static> AsRawFd for Device<D, A> { fn as_raw_fd(&self) -> RawFd { self.driver.as_raw_fd() } }
#[cfg(feature = "std")]
impl<D: Driver + AsFd, A: DeviceApi + 'static> AsFd for Device<D, A> { fn as_fd(&self) -> BorrowedFd<'_> { self.driver.as_fd() } }
