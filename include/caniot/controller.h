...
typedef enum {
	 ...
	 * "pq" can be NULL or set, "response" is VALID.
	 */
	CANIOT_CONTROLLER_EVENT_STATUS_ERROR, /* error frame received, is part of a
						 query (response is set) */
...
typedef struct {
	 ...
	 * Only valid in the context of a query
	 * (CANIOT_CONTROLLER_EVENT_CONTEXT_QUERY)
	 */
 ...
 *  	  the query should be cancelled if it doesn't get a response or if did
 * is BROADCAST
 * 	- otherwise a context is allocated and the query will
 * 	  be automatically cancelled after timeout
 * @return int handle on success (> 0), negative value on error, 0 if no context
 * allocated
 */
 ...
 *
 * Note: This function is equivalent to caniot_controller_query() with timeout =
 * 0
 *
 * @param ctrl
 * @param did
...
/**
 *
 * @brief Check timeouts and receive incoming CANIOT message if any and handle
 * it
 *
