...
static int cb_attr_write(struct caniot_device *dev,
		  uint16_t key,
		  uint32_t val)
{
...
const struct caniot_api api = CANIOT_API_FULL_INIT(
	cb_command_handler, cb_telemetry_handler,
	cb_config_on_read, cb_config_on_write,
	cb_attr_read, cb_attr_write);
