...
void caniot_show_deviceid(union deviceid did)
{
	if (caniot_valid_deviceid(did)) {
		if (CANIOT_DEVICE_IS_BROADCAST(did)) {
			...
			CANIOT_DBG(F("[%hhd] 0x%02x (cls=%s sid=%s)"),
			       did.val, did.val, cls, sid);
		}
	} else {
		CANIOT_DBG(F("invalid did"));
	}
}
...
{
	CANIOT_DBG(F("[ %x ] "), id.raw);
	if (caniot_is_error_frame(id) == true) {
		CANIOT_DBG(F("Error frame "));
		return;
	} else {
		CANIOT_DBG(F("%s %s "), get_type_str(id.type), get_query_str(id.query));
	}

	caniot_show_deviceid(CANIOT_DEVICE(id.cls, id.sid));

	CANIOT_DBG(F(" : %s / "), get_endpoint_str(id.endpoint));
}
...
void caniot_explain_frame(const struct caniot_frame *frame)
{
	...
	if ((frame->id.type == CANIOT_FRAME_TYPE_TELEMETRY) ||
		...
		for (int i = 0; i < frame->len; i++) {
			CANIOT_DBG(F("%02hhx "), (uint8_t) frame->buf[i]);
		}
	} else {
		CANIOT_DBG(F("LEN = %d, key = %02x val = %04lx"), frame->len,
...
{
	return snprintf(buf, len, "0x%02hhx", did.val);
}
