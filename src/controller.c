							   ...
							   const caniot_controller_event_t *ev)
{
	...
	__DBG("call_user_callback(ev: %p) -> did: %u handle: %u ctx: %u status: %u "
		  "term: "
		  "%u "
		  "resp: %p\n",
		  (void *)ev, ev->did, ev->handle, ev->context, ev->status, ev->terminated,
		  (void *)ev->response);
...
exit:
	__DBG("query(did: %u, frame: %p, timeout: %u, driv: %u) -> ret (handle): %d\n", did,
		  (void *)frame, timeout, (uint32_t)driv_send, ret);
									 ...
									 uint32_t timeout)
{
	...
	__DBG("caniot_controller_query_register(did: %u, frame: %p, timeout: %u) -> "
		  "ret: "
		  "%d\n",
							...
							uint32_t timeout)
{
	...
	__DBG("caniot_controller_query(did: %u, frame: %p, timeout: %u) -> ret "
		  "(handle): "
		  "%d\n",
		  did, (void *)frame, timeout, ret);
